...
const Navbar = () => {
    ...
    const menuItems = <>
        ...
        <li> {user ? <label htmlFor='confirmation' className='btn btn-primary rounded text-white ml-1' onClick={() => setModalData(true)}>Sign Out</label> : <NavLink className={`rounded ml-1`} to="/login">Login</NavLink>} </li>
    </>
    return (
        <>
            ...
            </div>
            {
                modalData && <Confirmation
                    ...
                    passData={logOut}
                    button={{ bg: 'btn-error', text: 'Sign Out' }}
                    message={'You\'re signing out!'}
                />
            }
        </>
    );
